            ...
            ZStack {
                ...
                ScrollView {
                    VStack(spacing: 30 * h) {
                        ...
                        // signup flow runs after the account is created and
                        // which root view (Clinician vs Patient) is shown
                        // after sign-in.
                        VStack(alignment: .leading, spacing: 8 * h) {
                            Text("I am a…")
                                .font(.system(size: 14 * w, weight: .medium))
...
// `completeProfile` then passes `nil` SurveyResponses, and the auth
